...
const handleImageUpload = async (req, res) => {
  try {
    ...
    const cldRes = await imageUploadUtil(dataURI);
    res.status(200).json({
      success: true,
...
const addProduct = async (req, res) => {
  try {
    ...
    const newlyCreatedProduct = new Product({
      ...
      productType,
      type,
      size: (category === "male" || category === "female") ? "XS" : size,
      measurement,
      percentageIncrement,
      price,
      salePrice,
...
const editProduct = async (req, res) => {
  try {
    ...
    findProduct.percentageIncrement = percentageIncrement || findProduct.percentageIncrement;
